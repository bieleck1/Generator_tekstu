...
void stworz_baze (baza_t *baza) {
	...
	baza->rozmiar = 10;
	baza->stat_wejsciowe = 0;
	baza->stat_wyjsciowe = 0;
	...
	for(i = 0; i < baza->rozmiar; i++) {
		...
		for(w = 0; w < 15; w++) {
			baza->prefixy[i].sufixy[w].ilosc_wejscia = 0;
			baza->prefixy[i].sufixy[w].ilosc_wyjscia = 0;
				...
				if (state_nowy_pref == 1) {
					...
					if (znak != '\n' && znak != ' ') {
						...
						if (w > 1 && dlugosc != 0) {
                                		...
                                		while ((znak = fgetc(in)) != EOF && znak != ' ' && znak != '\n' && w != 0) {
							prefix[dlugosc] = znak;
							dlugosc++;
				...
				if (znak != ' ' && znak != '.' && znak != '!' && znak != '?' && znak != '\n') {
					...
					for (w = 0; w < baza.aktualny_rozmiar; w++) {
						if (strcmp(baza.prefixy[w].prefix, prefix) == 0) {
							...
							if (baza.prefixy[w].ilosc_sufixow == 0 && strcmp(sufix, "") != 0) {
								...
								baza.prefixy[w].sufixy[0].ilosc_wejscia++;
								baza.stat_wejsciowe++;
								...
								for (g = 0; g < baza.prefixy[w].ilosc_sufixow && strcmp(sufix, "") != 0; g++) {
									if (strcmp(baza.prefixy[w].sufixy[g].sufix, sufix) == 0) {
										baza.prefixy[w].sufixy[g].ilosc_wejscia++;
										baza.stat_wejsciowe++;
								...
								if (state_sufix == 0 && strcmp(sufix, "") != 0) {
									...
									baza.prefixy[w].sufixy[baza.prefixy[w].ilosc_sufixow].ilosc_wejscia++;
									baza.stat_wejsciowe++;
									baza.prefixy[w].ilosc_sufixow++;
								}
							}
					...
					if (state_pref == 0) {
						...
						if (strcmp(sufix, "") != 0) {
							...
							baza.prefixy[baza.aktualny_rozmiar].sufixy[0].ilosc_wejscia++;
							baza.stat_wejsciowe++;
